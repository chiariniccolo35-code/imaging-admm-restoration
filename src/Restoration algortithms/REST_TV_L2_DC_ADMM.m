function [u,itrs] = REST_TV_L2_DC_ADMM(b,blur_k,n_sigma,tau,beta_t,beta_r,u,itrs_th,itrs_rel_chg_th,itrs_debugs,u_true)
%
% Compute approximate solutions (that is, constrained minimizers) of the
% discrepancy-constrained TV-L2 variational model: 
%
% u*(mu) = argmin J(u;delta),   J(u;delta) = TV(u) + i_B(delta)(K u - b),  
%             u
% with:   B(delta) = {x: ||x||_2 <= delta},  delta = tau sqrt(d) n_sigma
%
...
% INPUTS:
    % inputs defining the discrepancy-constrained TV-L2 cost function to be minimized:
        ...
        % tau       --> discrepancy parameter (positive real scalar)        
...
% --------------------
    ...
    % but that do not change during the ADMM iterations
        ...
        delta              = tau * sqrt(d) * n_sigma; % discrepancy constraint radius
...
if ( itrs_debugs == 1 )
    ...
    fprintf('\n---------------------------------------------------------');
    fprintf('\nTV-L2-DC-ADMM (tau = %6.4f) ITERATIONS-BASED DEBUGS:',tau);
    fprintf('\n---------------------------------------------------------');
    fprintf('\nIT %04d:  RC = %9.7f, J = %13.6f, tau* = %6.4f, [ISNR,ISSIM] = [%6.3f , %6.4f]',...
        0,0,J_regs(1),taus(1),snrs(1)-snrs(1),ssims(1)-ssims(1));
end
...
while ( sum(stop_flags) == 0 )
    ...
    % -----------------------------------------------------------------
    % solve the ADMM sub-problem for the primal variable r (projection)
    % -----------------------------------------------------------------
    r       = Ku_b + lambda_r;
    r_norm  = norm( r(:) ); 
    if ( r_norm > delta )
...
if ( itrs_debugs == 1 )
    ...
    figure('Name','Figure 80:  TV-L2-DC-ADMM:   ITERATIONS-BASED DEBUGS','NumberTitle','off');
    ...
    xlabel('k  (ADMM iteration index)');
    ylabel('$\mathrm{L}_2\left(u^{(k)};b,K\right) = \left\|Ku^{(k)}-b\right\|_2^2 / 2$','interpreter','latex');
    ...
    subplot(2,3,3)
    plot(0:itrs,J_regs,'r');
    xlabel('k  (ADMM iteration index)');
    ylabel('$J\left(u^{(k)};\right) = \mathrm{TV}\left(u^{(k)}\right)$','interpreter','latex');
    plot3_y_min = min( J_regs(2:end) );
    plot3_y_max = max( J_regs(2:end) );
