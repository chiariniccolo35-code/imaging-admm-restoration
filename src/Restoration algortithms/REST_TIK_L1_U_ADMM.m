function [u,itrs] = REST_TIK_L1_U_ADMM(b,blur_k,mu,beta_r,u,itrs_th,itrs_rel_chg_th,itrs_debugs,n_sigma,u_true)
%
% Compute approximate solutions (that is, minimizers) of the
% unconstrained TIK-L1 variational model: 
%
% u*(mu) = argmin J(u;mu),   J(u;mu) = (1/2)||D u||_2^2 + mu ||K u - b||_1  
%             u
%
...
% --------------------
    ...
    % given the inputs  b, blur_k, mu and beta_r,  for efficiency purposes 
    ...
    % but that do not change during the ADMM iterations
        ...
        SUB_X_CM_DFT       = (DhT_DFT .* Dh_DFT + DvT_DFT .* Dv_DFT)/beta_r + KT_DFT .* K_DFT;
...
while ( sum(stop_flags) == 0 )
    ...
    % ----------------------------------------------------------------
    q = Ku_b + lambda_r;
    r = sign(q) .* max( abs(q) - mu_over_beta_r , 0 );
    ...
    % --------------------------------------------------------------------
        ...
        rhs_DFT     = KT_DFT .* fft2(r - lambda_r + b);
...
if ( itrs_debugs == 1 )
    ...
    xlabel('k  (ADMM iteration index)');
    ylabel('$J\left(u^{(k)};\mu\right) = \mathrm{TIK}\left(u^{(k)}\right) + \mu \, \mathrm{L}_1\left(u^{(k)};b,K\right)$','interpreter','latex');
