function [u,itrs] = REST_TV_L2_U_ADMM(b,blur_k,mu,beta_t,u,itrs_th,itrs_rel_chg_th,itrs_debugs,n_sigma,u_true)
%
% Compute approximate solutions (that is, minimizers) of the
% unconstrained TV-L2 (also known as ROF) variational model: 
%
% u*(mu) = argmin J(u;mu),   J(u;mu) = TV(u) + mu ||K u - b||_2^2 / 2  
%             u
%
...
% INPUTS:
    % inputs defining the unconstrained TV-L2 cost function to be minimized:
...
% --------------------
    ...
    % given the inputs  b, blur_k, mu and beta_t,  for efficiency purposes 
    ...
    % but that do not change during the ADMM iterations
        ...
        mu_over_beta_t  = mu / beta_t; % constant in the coefficient matrix and in the right-hand side
        ...
        SUB_X_CM_DFT    = DhT_DFT .* Dh_DFT + DvT_DFT .* Dv_DFT + mu_over_beta_t * (KT_DFT .* K_DFT);
        SUB_X_RHS2_DFT  = mu_over_beta_t * ( KT_DFT .* fft2(b) );    
        
        % sub-problem for the dual variable lambda_t
        % ... nothing to pre-compute!
            
    % initialize other necessary quantities
    [Dhu,Dvu] = D(u); % gradient of the initial iterate u
...
if ( itrs_debugs == 1 )
    ...
    fprintf('\n---------------------------------------------------------');
    fprintf('\nTV-L2-U-ADMM (mu = %7.2f) ITERATIONS-BASED DEBUGS:',mu);
    fprintf('\n---------------------------------------------------------');
...
while ( sum(stop_flags) == 0 )
    ...
    % --------------------------------------------------------------------
        ...
        rhs_DFT     = fft2( DT(t_h - lambda_t_h,t_v - lambda_t_v) ) + SUB_X_RHS2_DFT;
...
if ( itrs_debugs == 1 )
    ...
    figure('Name','Figure 20:  TV-L2-U-ADMM:   ITERATIONS-BASED DEBUGS','NumberTitle','off');
    ...
    xlabel('k  (ADMM iteration index)');
    ylabel('$\mu \,\, \mathrm{L}_2\left(u^{(k)};b,K\right) = \mu \,\, \left\|Ku^{(k)}-b\right\|_2^2 / 2$','interpreter','latex');
    ...
    xlabel('k  (ADMM iteration index)');
    ylabel('$J\left(u^{(k)};\mu\right) = \mathrm{TV}\left(u^{(k)}\right) + \mu \, \mathrm{L}_2\left(u^{(k)};b,K\right)$','interpreter','latex');
