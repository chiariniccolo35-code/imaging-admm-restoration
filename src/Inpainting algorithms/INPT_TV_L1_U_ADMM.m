function [u,itrs] = INPT_TV_L1_U_ADMM(b,M,mu,beta_t,beta_r,u,itrs_th,itrs_rel_chg_th,itrs_debugs,n_sigma,u_true)
%
...
while ( sum(stop_flags) == 0 )
    ...
    % --------------------------------------------------------------------
        ...
        rhs   = DT(t_h - lambda_t_h,t_v - lambda_t_v) + ...
                      (beta_r_over_beta_t * M) .* (r - lambda_r + b);
