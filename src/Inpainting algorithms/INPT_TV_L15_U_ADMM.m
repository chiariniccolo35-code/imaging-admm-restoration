function [u,itrs] = INPT_TV_L15_U_ADMM(b,M,mu,beta_t,beta_r,u,itrs_th,itrs_rel_chg_th,itrs_debugs,n_sigma,u_true)
%
...
% --------------------
    ...
    % but that do not change during the ADMM iterations
        ...
        SUB_X_CM     = (Dh' * Dh + Dv' * Dv) + beta_r_over_beta_t * spdiags(M(:),0,d,d);
...
while ( sum(stop_flags) == 0 )
    ...
    % ----------------------------------------------------------------
    q_h                 = Dhu + one_over_beta_t * lambda_t_h;
    q_v                 = Dvu + one_over_beta_t * lambda_t_v;
    ...
    % --------------------------------------------------------------------
      ...
      % compute the right-hand side of the linear system
        ...
        rhs   = DT(t_h - lambda_t_h/beta_t, t_v - lambda_t_v/beta_t) + ...
                + (beta_r_over_beta_t * M ) .* (r - lambda_r/beta_r + b);

       
        % compute the new iterate
        u     = reshape( full( SUB_X_CM \ rhs(:) ) , size(b,1), size(b,2) );
...
if ( itrs_debugs == 1 )
    ...
    xlabel('k  (ADMM iteration index)');
    ylabel('$J\left(u^{(k)};\mu\right) = \mathrm{TV}\left(u^{(k)}\right) + \mu \, \mathrm{L}_1\left(u^{(k)};b,S\right)$','interpreter','latex');
