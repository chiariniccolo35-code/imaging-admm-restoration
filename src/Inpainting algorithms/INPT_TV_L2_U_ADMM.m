function [u,itrs] = INPT_TV_L2_U_ADMM(b,M,mu,beta_t,u,itrs_th,itrs_rel_chg_th,itrs_debugs,n_sigma,u_true)
%
% Compute approximate solutions (that is, minimizers) of the
% unconstrained TV-L2 (also known as ROF) variational model: 
%
% u*(mu) = argmin J(u;mu),   J(u;mu) = TV(u) + mu ||S u - b||_2^2 / 2  
%             u
%
...
% --------------------
    ...
    % given the inputs  b, M, mu and beta_t,  for efficiency purposes 
    ...
    % but that do not change during the ADMM iterations
        ...
        SUB_X_CM     = Dh'*Dh + Dv'*Dv + mu_over_beta_t * spdiags(M(:),0,d,d);
        SUB_X_RHS2   = mu_over_beta_t * sparse(b(:));
...
while ( sum(stop_flags) == 0 )
    ...
    % --------------------------------------------------------------------
        ...
        rhs   = DT(t_h - lambda_t_h,t_v - lambda_t_v);
        % compute the new iterate
        u     = reshape( full( SUB_X_CM \ (rhs(:)+SUB_X_RHS2) ) , h,w );
    ...
    if ( itrs_debugs == 1 )
        ...
        J_fids(itrs+1)     = (mu/2) * sum( Su_b_vec.^2 );
...
if ( itrs_debugs == 1 )
    ...
    xlabel('k  (ADMM iteration index)');
    ylabel('$\mu \,\, \mathrm{L}_2\left(u^{(k)};b,K\right) = \mu \,\, \left\|Su^{(k)}-b\right\|_2^2 / 2$','interpreter','latex');
    ...
    xlabel('k  (ADMM iteration index)');
    ylabel('$J\left(u^{(k)};\mu\right) = \mathrm{TV}\left(u^{(k)}\right) + \mu \, \mathrm{L}_2\left(u^{(k)};b,S\right)$','interpreter','latex');
